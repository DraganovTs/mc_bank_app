...
public class CustomerController {
    ...
    public ResponseEntity<CustomerDetailsDTO> fetchCustomerDetails(@RequestHeader("bankcorp-correlation-id") String correlationId,
                                                                   @RequestParam
                                                                   @Pattern(regexp = "^|[0-9]{10}"
                                                                           , message = "Mobile number must be 10 digits!")
                                                                   String mobileNumber) {
        logger.debug("bankcorp-correlation-id found: {}", correlationId);
